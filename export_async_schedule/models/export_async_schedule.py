...
class ExportAsyncSchedule(models.Model):
    ...
    email_subject = fields.Char(string="Sujet de l'e-mail")
    attachment_suffix = fields.Char(string="Suffixe pièce jointe")
    ...
    def _prepare_export_params(self):
        ...
        return {
            ...
            "attachment_suffix" :self.attachment_suffix
        }
    ...
    def action_cron_job(self):
        ...
        now = datetime.now()
        from_date = now - tolerance
        to_date = now + tolerance

        records = self.search([('next_execution', '>=', from_date), ('next_execution', '<=', to_date)])

        _logger.info('**** Exports : Records from %s To %s ' % (from_date, to_date))
        _logger.info('**** Exports : Records to process : %s ' % records.mapped('display_name'))
