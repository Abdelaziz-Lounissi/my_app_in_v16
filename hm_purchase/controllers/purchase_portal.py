...
def get_records_pager_my_purchase_orders(ids, current):
    ...
    if '/my/purchase/' in path:
        res['prev_record'] = idx != 0 and str(res['prev_record']).replace('/my/purchase', '/my/purchase')
        res['next_record'] = idx < len(ids) - 1 and str(res['next_record']).replace('/my/purchase', '/my/purchase')
...
class CustomerPortal(CustomerPortal):
    ...
    def _purchase_order_get_page_view_values(self, purchase_order_technicien, access_token, **kwargs):
    ...
    def portal_my_purchase_order(self, order_id=None, access_token=None, **kw):
        ...
        purchase_order_technicien = request.env['purchase.order'].sudo().browse(order_id)

        try:
            purchase_order_sudo = self._document_check_access('purchase.order', order_id, access_token)
    ...
    def generate_pdf(self, purchase_order_id=None, access_token=None, **kw):
        ...
        if purchase_order_id not in technicien_po.ids:
            ...
            pdf = request.env['ir.actions.report'].sudo()._render_qweb_pdf('purchase.action_report_purchase_order', [purchase_order_id])[0]
