...
class DelayExport(models.Model):
    ...
    user_ids = fields.Many2many(comodel_name="res.users", string="Users", index=True)
    ...
    def export(self, params):
        ...
        * user_ids: optional list of user ids who receive the file
        ...
        attachment_suffix = params.get('attachment_suffix', False)
        if attachment_suffix:
            name = "{}-{}.{}".format(model_name, attachment_suffix, export_format)
        ...
        subject = "Export {} {}".format(model_description, today)
        email_subject = params.get('email_subject', False)
        if email_subject:
            subject = "{} {}".format(email_subject, today)
