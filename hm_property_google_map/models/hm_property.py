...
class HmProperty(models.Model):
    ...
    def _compute_customer_geo(self):
        for lead in self:
            if lead.partner_id:
                lead.property_latitude = lead.partner_id.partner_latitude
                lead.property_longitude = lead.partner_id.partner_longitude
            else:
                lead.property_latitude = 0.0
                lead.property_longitude = 0.0
    ...
    def update_address(self, vals):
        ...
        if addr_vals:
            return super(HmProperty, self).write(addr_vals)
    ...
    def geo_localize(self):
        for lead in self.with_context(lang='en_US'):
            ...
            if result:
                lead.write(
                    {
                        'property_latitude': result[0],
                        'property_longitude': result[1],
                    }
                )
        return True
